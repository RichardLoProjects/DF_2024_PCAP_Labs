'''Lab for PCAP 4.3.1.16 @ 2024-10-25'''
...
def generate_histogram(letter_counts:dict) -> str:
    ...
    for letter in sorted(alphabet, key=lambda k:letter_counts[k], reverse=True):
...
def get_histogram() -> str:
    while True:
        try:
            ...
            histogram = generate_histogram(dict_data)
            save_histogram(file_name, histogram)
            return histogram
        except IOError as e:
            print(f'I/O error occurred: {strerror(e.errno)}')

def save_histogram(file_name:str, histogram:str) -> None:
    new_name = file_name + '.hist'
    with open(new_name, mode='wt') as file_stream:
